...
import { AgentConfig } from "#/dto/application";
...
const AgentApplicationForm: React.FC<AgentApplicationFormProps> = ({
  form,
}) => {
  ...
  const moveAgentUp = (index: number) => {
    ...
    [newSelectedAgents[index], newSelectedAgents[index - 1]] = [
      newSelectedAgents[index - 1],
  ...
  const availableAgents = agents.filter(
    (agent) => !selectedAgents.find((selected) => selected.agentId === agent.id)
  );
        ...
        <Row gutter={[24, 16]}>
          ...
          <Col span={12}>
            <Form.Item
              name="icon"
              label={t("应用图标")}
              rules={[{ type: "url", message: t("请输入有效的URL") }]}
            >
      ...
      >
        <Row gutter={[24, 16]}>
          <Col span={24}>
            <Form.Item
              label={t("添加Agent")}
              rules={[{ required: true, message: t("请至少选择一个Agent") }]}
            >
              <Select
