...
import { Button, Form, Modal, Select, Steps, Typography, message } from "antd";
...
const CreateApplicationModalNew: React.FC<CreateApplicationModalNewProps> = ({ visible, onCancel, onSuccess }) => {
	...
	const handleSubmit = async () => {
		try {
			...
			const applicationData: CreateApplicationDto = {
				...
				needModelSupport: values.needModelSupport !== undefined ? values.needModelSupport : false,

				// Agent相关参数
				agentConfigs: values.agentConfigs || [],
				kernelFunctionTerminationStrategy: values.kernelFunctionTerminationStrategy || "",
				kernelFunctionSelectionStrategy: values.kernelFunctionSelectionStrategy || "",
			...
			>
				{currentStep === 0 && (
					<div style={{ textAlign: "center", padding: "40px 0" }}>
						...
						<Form.Item name="applicationTypeId" rules={[{ required: true, message: t("请选择应用类型") }]}>
							<Select
				...
				<div className="mt-6 flex justify-end">
					{currentStep > 0 && (
						<Button style={{ margin: "0 12px" }} size="large" onClick={handlePrev}>
							{t("上一步")}
						</Button>
					)}
					{currentStep === 0 && (
						<Button type="primary" size="large" onClick={handleNext} disabled={!selectedAppType}>
							{t("下一步")}
						</Button>
					)}
					{currentStep === 1 && (
