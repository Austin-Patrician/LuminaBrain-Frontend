import { type FlowDataRaw, type FlowListParams, flowService } from "@/api/services/flowService";
...
import { Button, Card, Col, Input, Popconfirm, Row, Space, Statistic, Table, Tag, Typography, message } from "antd";
...
const AgentFlowListPage: React.FC = () => {
	...
	const columns: ColumnsType<FlowData> = [
		...
		},
		{
			...
			render: (_, record) => (
				<Space size="small">
					<Button type="link" size="small" icon={<EyeOutlined />} onClick={() => handleEditFlow(record.id || "")}>
						查看
					</Button>
					<Button type="link" size="small" icon={<EditOutlined />} onClick={() => handleEditFlow(record.id || "")}>
						编辑
					</Button>
					<Button
						type="link"
					...
					<Button type="link" size="small" icon={<CopyOutlined />} onClick={() => handleCopyFlow(record)}>
						复制
					</Button>
					<Popconfirm
						title="确定要删除这个流程吗？"
	...
	return (
		<div className="p-6 bg-gray-50 min-h-screen">
			...
			<Row gutter={16} className="mb-6">
				<Col xs={24} sm={6}>
					<Card>
						<Statistic title="总流程数" value={flows.length} valueStyle={{ color: "#1890ff" }} />
					</Card>
				</Col>
				<Col xs={24} sm={6}>
					<Card>
			...
			<Card className="mb-4">
				<div className="flex justify-between items-center">
					<div className="flex items-center gap-4">
						...
						{selectedRowKeys.length > 0 && (
							<div className="flex items-center gap-2">
								<Text className="text-gray-600">已选择 {selectedRowKeys.length} 项</Text>
					...
					<Button type="primary" icon={<PlusOutlined />} onClick={handleCreateFlow} size="large">
						创建新流程
					</Button>
				</div>
			</Card>
